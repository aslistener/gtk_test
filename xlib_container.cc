...
int test_main(int argc, char** argv) {
  ...
  std::thread reparent_thread([&](){
    ...
    cout << "d->fd : " << XConnectionNumber(display) << endl;
    cout << "xDisplay->fd : " << XConnectionNumber(xDisplay) << endl;
    cout << "GDK_WINDOW_XID(gtk_widget_get_window(window)) : " << GDK_WINDOW_XID(gtk_widget_get_window(window)) << endl;

    XReparentWindow(xDisplay, GDK_WINDOW_XID(gtk_widget_get_window(window)), container_window, 400, 400);
  });
  reparent_thread.detach();
    ...
    //     char** child_argv = (char **)calloc(argc + 1, sizeof(char* ));
    //     int a;
    //     for (a = 1; a < argc; ++a) {
